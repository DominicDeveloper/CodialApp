...
class MyMentorRecycleView(val context: Context, val list:List<Person>, val rvClick: RvClick) : RecyclerView.Adapter<MyMentorRecycleView.VH>() {
    
        inner class VH(var itemRv: View): RecyclerView.ViewHolder(itemRv){
            fun onBind(person: Person){

                 itemRv.item_list_items.text = person.curs_name.toString()
